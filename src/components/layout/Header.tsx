...
import { Search, Bell, ChevronDown, TrendingUp, TrendingDown, User, Settings, LogOut } from 'lucide-react';
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div
                    className={cn(
                      'text-[10px] font-bold font-mono',
                      pair.positive ? 'text-[var(--positive)]' : 'text-[var(--negative)]'
                    )}
                  >
                    {pair.change}
                  </div>
...
function AgentPill() {
  return (
    <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--accent-purple-glow)] border border-[var(--accent-purple)]/30 glow-purple">
...
function UserMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="w-9 h-9 rounded-xl bg-gradient-to-br from-[var(--accent-purple)] to-[var(--accent)] flex items-center justify-center font-bold text-sm text-white hover:scale-105 active:scale-95 transition-transform"
      >
        F
      </button>
      ...
      {open && (
        <div
          className="absolute top-full right-0 mt-2 w-56 rounded-2xl overflow-hidden z-50 animate-scale-in border border-[var(--border)] bg-[var(--bg-elevated)]"
          style={{ transformOrigin: 'top right' }}
        >
          <div className="px-4 py-3 border-b border-[var(--border)]">
            <p className="text-sm font-semibold text-[var(--text-primary)]">Fata Akromul</p>
            <p className="text-xs text-[var(--text-muted)]">fata@trading.io</p>
          </div>
          <div className="py-1">
            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-glass-light)] transition-colors">
          ...
          <div className="py-1 border-t border-[var(--border)]">
            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[var(--negative)] hover:bg-[var(--accent-rose)]/10 transition-colors">
